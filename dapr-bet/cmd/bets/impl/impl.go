...
func (s *BetsService) AddBet(ctx context.Context, in *common.InvocationEvent) (out *common.Content, err error) {
	...
	logger.Printf("new bet with ID: '%s' RaceName: '%s' RaceID: '%d' Amount: '%f' HorseID: '%d' HorseName: '%s' saved successfully for account '%s'", b.ID, b.RaceName, b.RaceID, b.Amount, b.HorseID, b.HorseName, b.Email)
	return out, nil
}
...
// GetBetsByEmail returns all bets for a account from the database
