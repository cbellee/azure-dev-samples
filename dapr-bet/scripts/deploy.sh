...
SERVICES=(bets results accounts races)
...
az acr build --registry $ACR_NAME \
    --image dapr-bet/accounts:${TAG} \
    --build-arg SERVICE_NAME="accounts" \
    --build-arg SERVICE_PORT=${ACCOUNTS_SERVICE_PORT} \
    --build-arg VERSION=${VERSION} \
    -f Dockerfile .
...
kubectl apply -f ../components/dapr-bet.binding.accounts.yml
kubectl apply -f ../components/dapr-bet.binding.results.yml
kubectl apply -f ../components/dapr-bet.statestore.bets.yml
kubectl apply -f ../components/dapr-bet.statestore.accounts.yml
